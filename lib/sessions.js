...
/**
 * The human-readable session title. IDE clients (Antigravity/Cursor) write a
 * `{type:"ai-title", aiTitle:"…"}` entry that evolves as the chat does — we take
 * the LAST one (most current), e.g. "Explore Fable model". Pure CLI sessions have
 * none → returns "" and callers fall back to the first message. Much nicer than
 * showing a raw first line like "<local-command-caveat>…" or "ok".
 */
function sessionTitle(lines) {
  for (let i = lines.length - 1; i >= 0; i--) {
    if (lines[i].indexOf('"ai-title"') === -1) continue;
    try { const o = JSON.parse(lines[i]); if (o && o.type === "ai-title" && o.aiTitle) return String(o.aiTitle).trim(); }
    catch { /* ignore */ }
  }
  return "";
}
...
/** Raw last-modified time (ms) of a session's transcript, or 0 if unknown. Two
 *  samples a moment apart reveal whether another client is writing it RIGHT NOW —
 *  a far more reliable "open elsewhere" signal than a single mtime snapshot. */
function sessionMtimeMs(dir, id) {
  const file = newestTranscript(dir, id);
  if (!file) return 0;
...
function limitFromTranscript(dir, id) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    const raw = lines[i];
    // Skip tool_result echoes: Claude Code writes a synthetic `user` line
    // carrying a tool_result AFTER the assistant turn. If we treated that as
    // "the newest turn", a rate_limit that the user is still sitting on would be
    // masked as not-limited — silently defeating watch mode. Skip them.
    if (/"tool_result"/.test(raw)) continue;
    let o;
    ...
    if (!isLimit) return { limited: false }; // newest real turn is normal -> not currently limited
...
/**
 * The N most-recently-active sessions across ALL projects (any client — Claude
 * Code, an IDE like Antigravity/Cursor, a terminal). Each entry carries its real
 * working directory (from the transcript's own `cwd`), so the app can resume it
 * directly. Used by the Start "which session?" picker so choosing a folder isn't
 * the only lever. Returns [{ id, dir, mtime, sizeKB, turns, preview, project }].
 */
function recentSessions(limit) {
  ...
  const cand = [];
  let folders = [];
  try { folders = fs.readdirSync(root); } catch { return []; }
  for (const folder of folders) {
    ...
    for (const f of files) {
      try {
        ...
        cand.push({ full: path.join(dir, f), id: f.replace(/\.jsonl$/, ""), mtime: st.mtimeMs, sizeKB: Math.round((st.size || 0) / 1024) });
      } catch { /* skip */ }
    }
  }
  cand.sort((a, b) => b.mtime - a.mtime);
  // Enrich only the top few (reading each transcript) rather than all of them.
  const out = [];
  for (const c of cand) {
    ...
    let cwd = "";
    for (const l of lines) {
      try { const o = JSON.parse(l); if (o && typeof o.cwd === "string" && o.cwd) { cwd = o.cwd; break; } } catch { /* ignore */ }
    }
    if (!cwd) continue; // no known working dir → can't resume it, skip
    const turns = lines.reduce((n, l) => n + (/"type":"user"/.test(l) && !/"tool_result"/.test(l) ? 1 : 0), 0);
    out.push({
      id: c.id, dir: cwd, mtime: new Date(c.mtime), sizeKB: c.sizeKB, turns,
      title: sessionTitle(lines), preview: firstUserMessage(lines).slice(0, 90), project: path.basename(cwd) || cwd,
    });
  }
  return out;
}
...
module.exports = {
  ...
  newestTranscript, sessionIdleMs, sessionMtimeMs, limitFromTranscript, recentSessions, sessionTitle,
};
