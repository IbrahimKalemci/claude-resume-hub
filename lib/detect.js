...
/**
 * Detect a terminal AUTH failure in Claude's output. This matters because Claude
 * Code can print "Authentication failed / Sign in again" AND still exit 0 — which
 * would otherwise be reported as a successful "task complete". Returns a friendly
 * message to show the user, or null.
 */
function detectAuthError(text) {
  if (!text) return null;
  // Specific CLI auth-failure phrases (kept tight to avoid matching a normal
  // answer that merely mentions "login"/"sign in").
  if (/authentication failed|sign in again|not logged in|invalid api key|unauthorized|please (?:log|sign)\s?in|oauth token (?:expired|invalid)/i.test(text)) {
    return "Claude isn't signed in. Run `claude auth login` in a terminal, then start again.";
  }
  return null;
}
...
module.exports = { detectLimit, detectAuthError, parseClockTime, fmtDuration };
