...
function detectAuthError(text) {
  if (!text) return null;
  if (/authentication failed|sign in again|not logged in|invalid api key|unauthorized|please (?:log|sign)\s?in|claude login|\/login\b|oauth token (?:expired|invalid)/i.test(text)) {
    return "Claude isn't signed in (authentication failed). Run `claude login` in a terminal, then start again.";
  }
  return null;
}
